...
cols = ['Year', 'Month', "Day", "Time_Hours", "Time_Minutes", "Theoretical_Power_Curve (KWh)", "Wind Speed (m/s)", "Wind Direction (°)"]
...
# create a differenced series -> trasforma i dati in una serie di differenze anzichè quantità assolute, per migliorare l'apprendimento
...
# invert differenced value -> riconverte i dati in scala originale
...
df_expectations = pd.DataFrame(expectations_plot, columns=["Expected"])
df_expectations.to_csv("expectations.csv", index=False)
print("Expectations salvate in expectations.csv")
